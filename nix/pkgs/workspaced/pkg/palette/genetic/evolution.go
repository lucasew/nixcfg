...
func initPopulation(r *rand.Rand, colors []api.LAB, count int, size int) []Individual {
	...
	for i := 0; i < size; i++ {
		...
		for j := 0; j < count; j++ {
			individual.colors[j] = api.LAB{
				L: r.Float64() * 100,        // Lightness: 0-100
				A: r.Float64()*200 - 100,    // Green-Red: -100 to +100
				B: r.Float64()*200 - 100,    // Blue-Yellow: -100 to +100
			}
		}
...
func mutate(r *rand.Rand, ind Individual, colors []api.LAB, rate float64) Individual {
	if len(colors) == 0 || r.Float64() > rate {
		return ind
	}
	...
	pos := r.Intn(len(mutated.colors))
	mutated.colors[pos] = colors[r.Intn(len(colors))]
...
func evolve(r *rand.Rand, survivors []scoredIndividual, imageColors []api.LAB) []Individual {
	...
	for i := 1; i < numSurvivors+numNewborns; i++ {
		...
		p1 := survivors[r.Intn(len(survivors))].individual
		p2 := survivors[r.Intn(len(survivors))].individual
		...
		if i > 0 {
			offspring = mutate(r, offspring, imageColors, mutationRate)
		}
