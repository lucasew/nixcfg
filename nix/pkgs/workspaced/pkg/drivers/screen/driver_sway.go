...
func (d *SwayDriver) Reset(ctx context.Context) error {
	if common.IsRiverwood() {
		...
		if err := common.RunCmd(ctx, "swaymsg", "output", "eDP-1", "mode", "1366x768", "pos", "0", "0").Run(); err != nil {
			return err
		}
		return common.RunCmd(ctx, "swaymsg", "output", "HDMI-A-1", "mode", "1366x768", "pos", "1366", "0").Run()
	}
	if common.IsWhiterun() {
		return common.RunCmd(ctx, "swaymsg", "output", "HDMI-A-1", "mode", "1368x768").Run()
	}
