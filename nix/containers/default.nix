...
let
  containerConf = {
    ...
    in
    unstable.dockerTools.streamLayeredImage (args' // {
      inherit name tag;
      maxLayers = 2;
      ...
      contents = [
        ...
        (dockerTools.fakeNss.override {
          extraPasswdLines = ["${user}:x:1000:1000:new ${user}:/state:/bin/bash"]
            ++ lib.optionals withNix (lib.genList (i: "nixbld${toString (i+1)}:x:${toString (i+30001)}:30000::/var/empty:/run/current-system/sw/bin/nologin") 32);
          extraGroupLines = ["${user}:x:1000:"]
            ++ lib.optional withNix "nixbld:x:30000:${lib.concatStringsSep "," (lib.genList (i: "nixbld${toString (i+1)}") 32)}";
        })
      ...
      config = {
        Entrypoint = [ (lib.getExe script-directory-wrapper) "source_me" ];
        User = user;
        Env = [
...
in builtins.mapAttrs (k: v: if v == "directory"
