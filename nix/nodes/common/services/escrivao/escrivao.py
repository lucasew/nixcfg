...
parser.add_argument(
    "--worker-socket", type=Path, default=Path("/run/python-microservices/stt-ptbr")
)
...
async def handle_audio(update: Update, context):
    items = [update.message.audio, update.message.document, update.message.voice]
    for item in items:
        if item is None:
            continue
        ...
        if file.file_size >= 20 * 1024 * 1024:
            await update.message.reply_text(
                "Erro: arquivo grande demais. Limite: 20MB",
        ...
        try:
            ...
            logger.info(
                f"audio ({update.message.chat.title}/{update.message.chat.username}-{update.message.chat.id}) {text}"
            )
