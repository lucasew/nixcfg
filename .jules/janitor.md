...
**Issue:** `bin/shim/workspaced` used fragile `cd` patterns (SC2103). CI failed due to `shfmt` errors in `bin/prelude/999-atuin.sh`, impure derivation usage (`teste-impure`) in `flake.nix`, `mise.toml` `lint:sh` task failing to find `shfmt`, and Go unused field linting errors.
**Root Cause:**
